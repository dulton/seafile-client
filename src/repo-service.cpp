...
public:
    ...
    const QString& repo_id() const { return repo_id_; }
    const QString& parent_repo_id() const { return parent_repo_id_; }
    const QString& parent_path() const { return parent_path_; }
...
public:
    ...
    bool operator()(const SyncedSubfolder &subfolder) {
        return subfolder.repo_id() == repo_id_;
    }
...
void RepoService::start()
{
    ...
    const char *sql = "CREATE TABLE IF NOT EXISTS SyncedSubfolder ("
        "repo_id TEXT PRIMARY KEY, parent_repo_id TEXT NOT NULL, "
        "url VARCHAR(24) NOT NULL, username VARCHAR(15) NOT NULL, "
        "parent_path TEXT NOT NULL)";
    sqlite_query_exec (synced_subfolder_db_, sql);

    sql = "CREATE INDEX IF NOT EXISTS SyncedSubfolderIndex ON SyncedSubfolder (url, username);";
    sqlite_query_exec (synced_subfolder_db_, sql);
...
void RepoService::onRefreshSuccess(const std::vector<ServerRepo>& repos)
{
    ...
    for (size_t i = 0; i < synced_subfolders_.size(); ++i) {
        ...
        for (size_t j = 0; j < server_repos_.size(); ++j) {
            if (synced_subfolders_[i].repo_id() == server_repos_[j].id) {
                found = true;
                server_repos_[j].parent_repo_id = synced_subfolders_[i].parent_repo_id();
                server_repos_[j].parent_path = synced_subfolders_[i].parent_path();
                break;
            }
        }
        // create GetRepoRequest
        if (!found)
            startGetRequestFor(synced_subfolders_[i].repo_id());
    }
...
void RepoService::onGetRequestSuccess(const ServerRepo& repo)
{
    ...
    if (pos != synced_subfolders_.end()) {
        ...
        fixed_repo.parent_path = pos->parent_path();
        fixed_repo.parent_repo_id = pos->parent_repo_id();
...
void RepoService::saveSyncedSubfolder(const ServerRepo& subfolder)
{
    ...
    QString sql = "REPLACE INTO SyncedSubfolder(repo_id, parent_repo_id, url, username, parent_path) VALUES ('%1', '%2', '%3', '%4', '%5')";
    sql = sql.arg(subfolder.id)
             .arg(subfolder.parent_repo_id)
             .arg(account->serverUrl.toEncoded().data())
             .arg(account->username)
             .arg(subfolder.parent_path);
